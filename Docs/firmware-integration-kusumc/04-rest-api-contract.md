# REST API Contract Map (Firmware/Operations)

## What changed since last refresh (2026-02-18)
- Expanded query-alias coverage notes to include status, pagination, cursor, and project-scope compatibility across admin/simulator/audit/device/vertical/analytics surfaces.
- Documented telemetry export filter alias support (`start/from`, `end/to`, `packetType/packet_type`, `quality/data_quality`, `exclude_quality/excludeQuality`).
- Consolidated conventions to reflect current wire policy: **snake_case is canonical**; legacy camelCase aliases may be accepted temporarily.
- Documented optional strict enforcement flag: `STRICT_SNAKE_WIRE=true` rejects camelCase query/JSON keys on `/api/*`.
- Added open-device command history fallback endpoint across `/device-open`, `/devices/open`, and `/v1` aliases.
- Added open-device command responses and command status fallback endpoints across `/device-open`, `/devices/open`, and `/v1` aliases.

## Source of truth
- Complete old-vs-new extracted route list: `docs/route-compare.md`.
- This file highlights firmware-adjacent and operations-critical routes.
...
| Device error ingest (MQTT fallback) | (none) | `POST /api/device-open/errors` (+ legacy + `/api/v1/...` aliases) | Body matches the device error schema (`open_id`, `timestamp`, `error_code`, `error_data`). Use this when MQTT publish to `<imei>/errors` fails. |
...
- Correlation rule: `msgid == correlation_id == config_id`
- Device must respond with the same `correlation_id` so the platform can finalize the configuration record.
...
`POST /api/v1/devices/:idOrUuid/configuration/ack` | Accepts an ack payload and marks the pending record acknowledged. New firmware should prefer MQTT response correlation; this route exists for compatibility and tools.
...
- For device configuration apply, treat `config_id` as the correlation ID and respond using `correlation_id=config_id`.
...
- Some endpoints may temporarily accept camelCase aliases like `deviceUuid`/`deviceId`, but new firmware and new tooling should not rely on them.

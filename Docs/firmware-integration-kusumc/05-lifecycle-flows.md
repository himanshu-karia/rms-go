...
- Reconfirmed lifecycle descriptions against the current legacy-first ingest/command flow (compat topics are optional and disabled by default).
...
3. Device applies configuration and publishes ack/resp with `correlation_id=config_id`.

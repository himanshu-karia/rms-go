...
Chosen format for new firmware and new tooling:
- `/api/device-open`
- `/api/v1/device-open`

Legacy aliases (supported for backward compatibility; do not use for new firmware if avoidable):
- `/api/devices/open`
- `/api/v1/devices/open`
...
- Legacy camelCase aliases may still be accepted by some endpoints during migration, but new firmware should not rely on them.
...
```json
{
  ...
  "credentials": {
    ...
    "publish_topics": [
      ...
      "channels/prj-01/messages/359762081234567",
      "devices/359762081234567/telemetry",
      "devices/359762081234567/errors"
    ],
...
```json
{
  ...
  "correlation_id": "<config_id_uuid>",
  "command": {
    "name": "apply_device_configuration",
    "params": {
...
- If the broker echoes publishes to the same client subscription, ignore your own `ondemand_rsp` publishes (respond only to payloads that contain `command` or `packet_type=ondemand_cmd`).

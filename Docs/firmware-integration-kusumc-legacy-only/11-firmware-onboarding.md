...
- Recommended session settings (MQTT client):
  ...
  - LWT (Last Will and Testament): optionally publish an "offline" heartbeat-shaped payload to the telemetry uplink topic so operators can see ungraceful disconnects.
    - Topic: `IMEI/heartbeat` (and other suffix topics like `/data`, `/daq`)
...
- Device should publish ack/resp with correlation:
  - Prefer echoing `msgid` from the command (strongly recommended). Some govt responses omit explicit correlation keys.

Command contract (server -> device):
- Govt legacy OnDemandCommand (top-level keys):
  - `msgid`
  - `timestamp`
  - `type=ondemand_cmd`
  - `cmd` (command name)
  - command params at top level (for example `interval_sec`)

Response contract (device -> server):
- Govt legacy OnDemandResponse uses `status` and may include `code`:
  - Strong recommendation: include `msgid` equal to the command `msgid`.
  - `status`: `ack` | `wait` | `failed`
  - Optional numeric `code` mapping (when used): `0=ack`, `1=failed`, `2=wait`
...
- Last `msgid` published and last command `msgid` executed

...
NOTE: This file is primarily for **bench validation** and platform support. For firmware implementation, use `for-firmware-agent/`.
...
3) Telemetry publishes on `primary_broker.publish_topics[]` (PumpData uplink topic: `<imei>/data` only)
4) Command arrives on `primary_broker.subscribe_topics[0]`
5) Device publishes ondemand ack/resp with `status` and echoes command `msgid` (recommended)
...
- Telemetry publish: `<imei>/{heartbeat|data|daq}`
...
- Govt legacy command shape (top-level keys):
  - `msgid`
  - `timestamp`
  - `type=ondemand_cmd`
  - `cmd` (command name)
  - command params at top level
...
- `code` (optional numeric mapping)
- `msgid` equal to the command `msgid` (strong recommendation)

...
NOTE: This document is **platform/server verification** material (legacy vs Go parity) and is **not required** for legacy-only firmware development.
...
- Success bar: preserve legacy endpoints/topic behavior via exact parity or aliases.
...
| Telemetry MQTT ingest | Topic suffix-centric (`<imei>/<suffix>`) | Legacy topics supported with PumpData on data lane (`<imei>/{heartbeat,data,daq,ondemand}`), historical `/pump` accepted for compatibility | Mirrored | None |
...
- Yes. This documentation assumes legacy device contracts should keep working through aliases/config before requiring firmware rewrites.
